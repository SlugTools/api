...
def scrape_menus(date):
    ...
    for i in locations['managed']:
        ...
        for j in locations['managed'][i]:
            ...
            new = session.get(f'https://nutrition.sa.ucsc.edu/shortmenu.aspx?locationNum={j:02d}&locationName={quote_plus(locations["managed"][i][j]["name"])}&naFlag=1&dtdate={date}')

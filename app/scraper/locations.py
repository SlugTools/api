...
def scrape_locations():
    ...
    for i in managed:
        ...
        isMultiple = [False, []]
        for j in matches:
            ratio = fuzz.ratio(managed[i]['name'].lower().replace('dining hall', ''), j.text.lower().replace('dining hall', ''))
            ratio_list[ratio] = j
            if managed[i]['name'].lower()[:-1] in j.text.lower() and 'closed' not in j.text.lower():
        ...
        for j in data['data']:
            ...
            if len(ratio_list) == 4:
                break
            ratio = fuzz.ratio(managed[i]['name'].lower().replace('dining hall', ''), j['name'].lower().replace('dining hall', ''))
        ...
        if isMultiple[0]:
            ...
            for j in isMultiple[1]:
                managed[i]['description'].append(sub(' +', ' ', normalize('NFKD', matches[j].find('p').text.split('✆')[0].strip())))
                # https://developers.google.com/maps/documentation/urls/get-started; maybe parse web embed url
                managed[i]['location'].append(f'https://www.google.com/maps/dir/?api=1&destination={quote_plus(j.text.strip())}')
                managed[i]['phone'].append(sub(' +', ' ', normalize('NFKD', matches[j].find('p').text.split('✆')[1].strip())))
        else:
            managed[i]['description'] = sub(' +', ' ', normalize('NFKD', li.find('p').text.split('✆')[0].strip()))
            managed[i]['location'] = f'https://www.google.com/maps/dir/?api=1&destination={quote_plus(managed[i]["name"])}'
            managed[i]['phone'] = sub(' +', ' ', normalize('NFKD', li.find('p').text.split('✆')[1].strip()))
        ...
        if max_ratio == 46 or max_ratio > 90:
            if ratio_list[max_ratio][0]['isOpen']:
                ...
                managed[i]['occupation'] = {
                    ...
                    'busyness': {
                        'status': ' '.join(ratio_list[max_ratio][0]['locHtml']['summary'].split(' ')[:2]),
                        'percent': int(findall(r'\d+', ratio_list[max_ratio][0]['locHtml']['summary'])[0])
                    },
                    'bestLocation': None,
                    'subLocations': None,
                    'trends': None
                    ...
                    if ratio_list[max_ratio][0]['subLocs']:
                        ...
                        for j in ratio_list[max_ratio][0]['subLocs']:
                            ...
                            managed[i]['occupation']['subLocations'].append({
                                ...
                                'capacity': j['capacity'],
                                'busyness': {
                                    'status': ' '.join(j['subLocHtml']['summary'].split(' ')[:2]),
                                    'percent': int(findall(r'\d+', j['subLocHtml']['summary'])[0])
                                },
                            })
                trends = []
                for j in ratio_list[max_ratio][1]['comparison']:
    ...
    for k, v in matches.items():
        unmanaged['other'].append({
            'name': k.text,
            'description': normalize('NFKD', ''.join([i.text for i in v[2:]])).strip().replace('\n', ' ').replace('    ', '. '),
    ...
    for i in managed: master['managed']['diningHalls' if 'hall' in managed[i]['name'].lower() else 'butteries'][i] = managed[i]

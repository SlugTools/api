...
async def main():
    async with ClientSession() as session:
        resp = await session.get("https://www.laundryview.com/api/c_room?loc=9632", ssl = False)
        campus = await resp.json()
        final = {}
        for i in range(len(campus["room_data"])):
            id = campus['room_data'][i]['laundry_room_location']
            resp = await session.get(f"https://www.laundryview.com/api/currentRoomData?location={id}", ssl = False)
            ...
            for k in location["objects"]:
                if k.get("appliance_type"):
                    ...
                    individual[type].append({
                        "status": k["time_left_lite2"] if k.get("time_left_lite2") else k["time_left_lite"]
                    })

...
srcs = {
    ...
    # "Campus Directory": "https://campusdirectory.ucsc.edu/cd_simple",
    "Pisa Class Search": "https://pisa.ucsc.edu/class_search/",
    "ITS Classroom Media": "https://its.ucsc.edu/classrooms/",
    # "TextbookX": "https://ucsc.textbookx.com/institutional/index.php"
}
...
def classrooms_name(name: str):
    return rooms_name(name)


@bp.route("/rmp/<string:name>")
def rmp(name: str):
    """
...
def term():
    """
    Code for academic term (for class search)<br>
...
@bp.route("/classes/search/<string:code>")
@limiter.limit("5/minute")
...
def classes_search_template():
    """
    Headers template for <a href=/catalog/classes/search>/classes/search</a></code>
    """
    template = melt(catalogDB.get("template"))
    for i in template:
...
def courses_search_template():
    return classes_search_template()
